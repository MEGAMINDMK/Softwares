...
#include "game_III/CPed.h"
#include "game_III/CPlayerPed.h"
#include "game_III/CEntity.h"
...
private:
    ...
    void HandleIdle(CPlayerPed* player) {
        if (KeyPressed('E')) {
            if (CPed* ped = FindNearestPed(player)) {
                ...
                CHud::SetHelpMessage(const_cast<wchar_t*>(L"Press ~y~Y~w~ to chat"), true);
    ...
    void HandlePrompted() {
        ...
        if (KeyPressed('Y')) {
            ...
            CHud::SetHelpMessage(const_cast<wchar_t*>(L"Type your message (~g~ENTER~w~ to send)"), true);
    ...
    void UpdateDisplay() {
        ...
        CHud::SetHelpMessage(const_cast<wchar_t*>(display.c_str()), true);
    }
    ...
    void Reset() {
        m_pInteractingPed = nullptr;
        m_eState = STATE_IDLE;
        CHud::SetHelpMessage(const_cast<wchar_t*>(L""), true);
    }
} pedInteraction;