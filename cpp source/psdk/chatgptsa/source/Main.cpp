...
private:
    ...
    void HandleIdle(CPlayerPed* player) {
        if (KeyPressed('E')) {
            if (CPed* ped = FindNearestPed(player)) {
                ...
                CHud::SetHelpMessage("Press ~y~Y~w~ to chat", true, false, false);
    ...
    void HandlePrompted() {
        ...
        if (KeyPressed('Y')) {
            ...
            CHud::SetHelpMessage("Type your message (~g~ENTER~w~ to send)", true, false, false);
    ...
    void MakePedFacePlayer(CPed* m_pInteractingPed, CPlayerPed* player) {
        ...
        m_pInteractingPed->m_fCurrentRotation = headingRad;
    ...
    void UpdateDisplay() {
        std::string display = "Typing: " + m_sCurrentInput;
        CHud::SetHelpMessage(display.c_str(), true, false, false);
    }

    void DisplayResponse(const std::string& response) {
        CMessages::AddMessage(const_cast<char*>(response.c_str()), 5000, 1, false);
    }
    ...
    CPed* FindNearestPed(CPlayerPed* player) {
        ...
        auto* pool = CPools::ms_pPedPool;  // Correct type: CPool<CPed, CCopPed>*
    ...
    void Reset() {
        m_pInteractingPed = nullptr;
        m_eState = STATE_IDLE;
        CHud::SetHelpMessage("", false, false, false);
    }
} pedInteraction;